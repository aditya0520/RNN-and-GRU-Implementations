    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            for layer in range(len(self.rnn)):
                if layer == 0:
                    hidden[layer] = self.rnn[layer].forward(x[:,t,:], hidden[layer,:,:])
                else: 
                    hidden[layer] = self.rnn[layer].forward(hidden[layer-1,:,:], hidden[layer,:,:])
            self.hiddens.append(hidden.copy())
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in reversed(range(seq_len)):
            for layer in reversed(range(len(self.rnn))): 
                ...
                dx_t, dh_t = self.rnn[layer].backward(dh[layer], self.hiddens[t+1][layer], h_prev_l, h_prev_t)
